 ...
 !       if (currentPatch%active_crown_fire == 1) then
       ...
       !loop across cohorts to calculate canopy fuel load by 1m height bin
        currentCohort=>currentPatch%tallest
           ...
           currentPatch%canopy_bulk_density = sum(biom_matrix[int((height_base_canopy-1.0_r8)):]) / &
           (max_height - height_base_canopy)
       ...
       if (currentPatch%fire == 1) then
          ...
          do while(associated(currentCohort))  
             ...
             if ( prt_params%woody(currentCohort%pft) == itrue) then !trees only
                ...
                if ((currentCohort%height > 0.0_r8).and.(currentPatch%Scorch_ht(currentCohort%pft) >= height_cbb)) then 
